...
export function Paging({ offset, limit, total, change }: { offset: number; limit: number; total: number; change: (n: number) => void }) {
  return <div className="actions">
    <button type="button" className="secondary" disabled={offset === 0} onClick={() => change(Math.max(0, offset - limit))}>Anterior</button>
    <span>{total === 0 ? 0 : offset + 1}–{Math.min(total, offset + limit)} de {total}</span>
    <button type="button" className="secondary" disabled={offset + limit >= total} onClick={() => change(offset + limit)}>Siguiente</button>
  </div>;
}
