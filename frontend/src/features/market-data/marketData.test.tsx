...
const result: CatalogResult = { metadata: { ...dataset, start: dataset.first_candle, end: dataset.last_candle, pattern_length: 3, include_doji: true }, candles_examined: 30, eligible_windows: 24, windows_skipped_due_to_gaps: 3, windows_skipped_due_to_doji: 0, limitation: "Historical only", patterns: [{ pattern: "CCC", pattern_length: 3, sample_size: 6, next_bullish_count: 0, next_bearish_count: 6, next_doji_count: 0, next_bullish_probability: "0", next_bearish_probability: "1", next_doji_probability: "0", first_observation: dataset.first_candle, last_observation: dataset.last_candle, distinct_days: 2 }] };
beforeEach(() => {
  vi.resetAllMocks();
