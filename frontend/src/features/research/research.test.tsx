...
it('creates a private draft without a profitability claim',async()=>{
  mount(<StrategyLab/>);
  fireEvent.change(screen.getByLabelText("Nombre"),{target:{value:'Research Example'}});
  fireEvent.change(screen.getByLabelText("Descripción"),{target:{value:'Manual'}});
  fireEvent.click(screen.getByRole('button',{name:"Crear estrategia"}));
  await screen.findByRole('link',{name:'Research Example'});
  expect(researchApi.create).toHaveBeenCalledWith('Research Example','Manual');
  expect(screen.getByText(/DENTRO DE LA MUESTRA · SIN VALIDAR/)).toBeTruthy();
});
it('builds declared indicators, numeric and field-to-field conditions, direction and bars_ago as a new version',async()=>{
  ...
  await waitFor(()=>expect((screen.getByRole('button',{name:"Añadir indicador"}) as HTMLButtonElement).disabled).toBe(false));
  fireEvent.click(screen.getByRole('button',{name:"Añadir indicador"}));
  await screen.findByRole('combobox',{name:"Indicador 1"});
  fireEvent.click(screen.getByRole('button',{name:"Añadir indicador"}));
  fireEvent.change(screen.getByRole('combobox',{name:"Indicador 2"}),{target:{value:'EMA'}});
  fireEvent.click(screen.getByRole('button',{name:"Añadir indicador"}));
  fireEvent.change(screen.getByRole('combobox',{name:"Indicador 3"}),{target:{value:'EMA'}});
  fireEvent.change(screen.getByLabelText("Período 3"),{target:{value:'50'}});
  await screen.findByRole('option',{name:'rsi_14'});
  fireEvent.change(screen.getByRole('combobox',{name:"Campo 1"}),{target:{value:'rsi_14'}});
  fireEvent.change(screen.getByRole('combobox',{name:"Operador 1"}),{target:{value:'GTE'}});
  fireEvent.change(screen.getByLabelText("Valor 1"),{target:{value:'70'}});
  fireEvent.click(screen.getByRole('button',{name:"Añadir condición"}));
  fireEvent.change(screen.getByRole('combobox',{name:"Campo 2"}),{target:{value:'ema_20'}});
  fireEvent.change(screen.getByRole('combobox',{name:"Tipo de operando 2"}),{target:{value:'FIELD'}});
  fireEvent.change(screen.getByRole('combobox',{name:"Campo derecho 2"}),{target:{value:'ema_50'}});
  fireEvent.click(screen.getByRole('button',{name:"Añadir condición"}));
  fireEvent.change(screen.getByRole('combobox',{name:"Campo 3"}),{target:{value:'direction'}});
  fireEvent.change(screen.getByLabelText("Velas anteriores 3"),{target:{value:'2'}});
  await waitFor(()=>expect((screen.getByRole('button',{name:"Crear versión"}) as HTMLButtonElement).disabled).toBe(false));
...
it('runs only explicitly with fixed payout/snapshot/expiry config and navigates',async()=>{
  ...
  fireEvent.change(screen.getByRole('combobox',{name:"Conjunto de datos"}),{target:{value:JSON.stringify(dataset)}});
  fireEvent.change(screen.getByLabelText("Rendimiento %"),{target:{value:'83.5'}});
  fireEvent.change(screen.getByLabelText("ID de vela de corte"),{target:{value:'100'}});
  expect(researchApi.run).not.toHaveBeenCalled();
  expect(screen.getByText(/Supuesto de rendimiento fijo: 83.5%/)).toBeTruthy();
  fireEvent.click(screen.getByRole('button',{name:"Iniciar prueba histórica"}));
...
it('renders in-sample result, exact unit evidence, equity and trade inspector',async()=>{
  ...
  await screen.findByText(/Supuesto de rendimiento fijo: 83.5%/);
  expect(screen.getByText(/DENTRO DE LA MUESTRA · SIN VALIDAR/)).toBeTruthy();
  expect(screen.getByRole('img',{name:"Curva de resultado acumulado en unidades"})).toBeTruthy();
  expect(screen.getByText("ID de vela de corte")).toBeTruthy();
  fireEvent.click(await screen.findByRole('button',{name:"Inspeccionar n.º1"}));
  expect(screen.getByRole('region',{name:"Contexto de señal"}).textContent).toContain('close@0');
});
it('shows API errors without inventing completed results',async()=>{
  ...
  mount(<StrategyLab/>);
  fireEvent.change(screen.getByLabelText("Nombre"),{target:{value:'Research'}});
  fireEvent.click(screen.getByRole('button',{name:"Crear estrategia"}));
