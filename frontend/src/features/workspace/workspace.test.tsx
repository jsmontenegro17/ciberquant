...
it('shows loading without stale evidence from another selection',()=>{
 ...
 renderContext();expect(screen.getByText("Cargando evidencia de investigación…")).toBeTruthy();
 expect(screen.queryByRole('list',{name:"Etapas de investigación"})).toBeNull();
});
...
it.each(['READY','MISSING','RUNNING','FAILED','STALE','INCOMPATIBLE','UNAVAILABLE'])('renders pipeline %s without an execution recommendation',status=>{
 const data:Overview={dataset:{source:'IQOPTION',broker:'IQOPTION',symbol:'EURUSD-OTC',market_type:'OTC',timeframe:'1m'},version:{id:17},provider_health:[],evidence:{manual:null,validation:null},comparison_note:"Muestras remain separate.",pipeline:[{name:'DATA',status,href:'/market-data',date:null,evidence:{id:1}}]};
 render(<MemoryRouter><Pipeline data={data}/></MemoryRouter>);
 expect(screen.getByText(codeLabel(status))).toBeTruthy();
 expect(screen.getByText(/Versión de estrategia n.º17/)).toBeTruthy();
 expect(screen.getByRole('link',{name:'Datos (DATA)'}).getAttribute('href')).toBe('/market-data');
...
it('empty datasets and versions never auto-select a context',async()=>{
 ...
 expect(await screen.findByText(/No hay conjuntos de datos/)).toBeTruthy();
 expect(screen.getByText(/No has elegido un contexto/)).toBeTruthy();
 expect(screen.getByRole('link',{name:"Importar datos de mercado"}).getAttribute('href')).toBe('/market-data');
});
