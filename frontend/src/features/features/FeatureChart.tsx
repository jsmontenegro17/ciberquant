...
export function FeatureChart({ result }: { result: FeatureResult }) {
  ...
  useEffect(() => {
    ...
    createSeriesMarkers(candles, result.rows.filter(r => r.gap_before).map(r => ({ time: time(r), position: 'aboveBar', shape: 'circle', color: '#eab45c', text: 'Gap' })));
    ...
    result.feature_keys.filter(k => /^(sma_|ema_|rsi_|atr_|bb_(middle|upper|lower)_)/.test(k)).forEach((key, i) => {
      ...
      if (rsi) [30, 70].forEach(price => line.createPriceLine({ price, color: '#7d8791', lineWidth: 1, lineStyle: 2, axisLabelVisible: true, title: 'Reference' }));
    });
  ...
  return <div>
    <div ref={container} style={{ height: 620 }} aria-label="Feature candlestick chart" />
    <p>RSI: 0–100; 30/70 are visual references only. ATR: observed volatility. Yellow dots: data gaps.</p>
    <small>Chart by <a href="https://www.tradingview.com/" target="_blank" rel="noreferrer">TradingView Lightweight Charts™</a>. Rendering uses browser numbers; authoritative decimal strings appear below.</small>
    {row && <div aria-label="Candle details"><p>{row.open_time} UTC · O {row.open} · H {row.high} · L {row.low} · C {row.close} · {row.direction} · Gap {row.gap_before ? row.gap_seconds + 's' : 'no'} · Run {row.contiguous_run_length}</p>
