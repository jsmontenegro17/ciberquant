...
Status: RESOLVED
REQ-008 implementation update (Human Acceptance PENDING): Essential abandoned-import reconciliation implemented with PostgreSQL active lifecycle ownership, exclusive recovery and audited FAILED transition; concurrency/idempotency/active-job tests. Raw candles untouched, explicit reimport remains idempotent. Background queues are not required or introduced.
