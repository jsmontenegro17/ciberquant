...
## Final human acceptance — 2026-09-19

Approved HEAD: e381084b0cace7eab2fb29b06dfd112131d4a1a5. No P0/P1 in accepted scope. Human review accepts real PRACTICE IQ binary/OTC smoke at2026-09-19T21:49:03.407515Z, research precedence, causal paper outcomes and zero orders/ledger writes. The following earlier QA notes are historical; this acceptance supersedes their pending-review gates.

Freeze cq-live-data-v1 and cq-scanner-v1, including resolve_paper_config. Dependencies cq-features-v1, cq-strategy-dsl-v1, cq-binary-backtest-v1 and cq-validation-v1 remain frozen. No incompatible semantic changes under these identifiers. Binary is verified; digital unsupported. PRACTICE mandatory; REAL rejected. Pin acac6e08333466ae188c7dfa7fd2a03174e34ca2 retained.

Release procedure: final documentation/version commit → feature CI PASS → PR #6 merge → main CI PASS → v0.7.0 tag on validated main. Exact integrated SHA and CI run ID are recorded in the PR #6 release handoff and annotated tag (avoids changing the validated commit to record its own SHA). Stop after closure; REQ-008 requires separate authorization.
...
Pinned upstream SHA acac6e08333466ae188c7dfa7fd2a03174e34ca2. Private async read-only wire allowlist, PRACTICE/no REAL fallback, secret redaction even at DEBUG, Decimal parsing, product-specific metadata, verified dynamic capabilities, pagination/clock checks, optional installation. Backend Docker build now excludes local credentials. Original research precedence tests remain intact. Local final verification and exact final CI SHA/run are recorded in the PR handoff; only the final HEAD green checks are merge evidence. Human Acceptance PASS; release closure authorized, REQ-008 not started.

Human review requested an explicit pure paper-config resolver: normal mode uses validation expiry and provider payout with validation fallback; research mode uses research expiry and provider payout with research fallback. Historical validation remains reference metadata, never hidden research execution configuration. Add payout/expiry source evidence and truthful UI labels; regress all four cases and actual paper outcomes. Correction accepted; no engine semantic change or REQ-008. Corrected-head CI evidence will be recorded in PR #6 and the handoff report.
...
No known P0/P1 found in automated checks; independent human QA completed PASS. P2 [LIVE-001](../debt/LIVE-001-provider-operations.md), existing SEC-001 and quant/recovery debts, dependency warnings and bundle-size warning. Real IQ OTC is implemented and smoke-verified in REQ-007; no REQ-008 implementation. Human acceptance PASS; merge/tag only after final CI.
...
Automated acceptance A–J: PASS. Status DONE; Human Acceptance PASS. Validate final feature CI, merge, validate main and tag v0.7.0; no REQ-008.
